...
app.config([
  ...
  function ($routeProvider, $locationProvider) {
    $routeProvider.when('/home', {
      ...
      resolve: { toAllowOrNot: checkStatus }
    }).when('/login', {
...
function check_login(q, location, http, Session) {
  ...
  function resolve_deferred(deferred) {
    if (location.path() !== '/user/update' && Session.data['userinfo']['username'] != Session.currentuser['username']) {
...
checkStatus = [
  ...
  function ($q, $location, $http, Session) {
    return Session.defferred.promise.then(function (response) {
      return Session.assetsotherinfodefferred.promise;
    }).then(function (response) {
      return Session.assetsinfodefferred.promise;
    }).then(function (response) {
      return check_login($q, $location, $http, Session);
    });
  }
];
      ...
      for (i = 0; i < subcategories.length; ++i) {
        if ($scope.categorydetails.entereddetails.subcategoryname == subcategories[i].subcategoryname) {
          ...
          for (i = 0; i < categories.length; ++i) {
            if (categories[i].categoryid == categoryid && $scope.categorydetails.entereddetails.categoryname == categories[i].categoryname) {
      ...
      if (foundcombination || riskname == null) {
        ...
        $http({
          ...
          if (data) {
            ...
            $modalInstance.close(entereddetails);  /* TODO - Might need to broadcast when update of category and subcategory is done */
          }
        });
      }
    };
...
app.controller('navbarController', [
  ...
  '$rootScope',
  'Session',
  function ($scope, $http, $location, $window, $rootScope, Session) {
...
app.controller('groupingsController', [
  '$scope',
  '$rootScope',
  '$http',
  '$filter',
  '$modal',
  'Session',
  function ($scope, $rootScope, $http, $filter, $modal, Session) {
    Session.assetsotherinfodefferred.promise.then(function (response) {
      return Session.assetsinfodefferred.promise;
    }).then(function (response) {
      return Session.initialassstsupdate.promise;
    }).then(function (response) {
    ...
    $scope.updateAssetsData = function (firstload) {
      ...
      $scope.submitchanges = function (parentindex, index) {
        ...
        if (d.extra.categoryname != $scope.data[index].extra.categoryname || d.extra.subcategoryname != $scope.data[index].extra.subcategoryname) {
          ...
          if (categoryid == null) {
            ...
            subcategories = $scope.assetsOtherInfo.subcategories;
            for (i = 0; i < subcategories.length; ++i) {
              if (categoryid == subcategories[i].categoryid && entereddetails.subcategoryname == subcategories[i].subcategoryname) {
                subcategoryid = subcategories[i].subcategoryid;
                break;
              }
            }
...
app.controller('userController', [
  ...
  'Session',
  '$routeParams',
  function ($scope, $http, $rootScope, $timeout, $location, Session, $routeParams) {
     ...
     */
    $scope.setPassword = $scope.setPassword && !$scope.register;
    // when register is true, it will always override setPassword! When false it need not override
